...
from docx.enum.text import WD_PARAGRAPH_ALIGpytonNMENT
...
#os.chdir("D:/IPYNB/KSFE2023/")
#form = Document(docx = "D:\IPYNB\KSFE2023\INDATEN\Tables.docx")
#df = pd.read_csv('D:\\IPYNB\\KSFE2023\\DATEN\\Export.csv')
#country = pd.read_csv('D:\\IPYNB\\KSFE2023\\INDATEN\\Country1.csv')
#input_doc = Document('D:\\IPYNB\\KSFE2023\\INDATEN\\Para1.docx')
#front_doc = Document('D:\\IPYNB\\KSFE2023\\INDATEN\\front.docx')
#text_file = open('D:\\IPYNB\\KSFE2023\\INDATEN\\TxtGrVor.txt', "r")
...
def tab_copy(k): 
    s=22
    p2 = doc.add_heading("Tabelle â„–" + str(s+k+1) , 3) 
    table = form.tables[k]
    table.alignment = WD_TABLE_ALIGNMENT.LEFT
    tbl = table._tbl
    paragraph = doc.add_paragraph("Hier wird die Tabelle â„–" + str(s+k+1) + " kopiert! Alle Daten wurde Simuliert!")
...
def update_toc0(docx_file):
    ...
    #wd_section.Footers(constants.wdHeaderFooterPrimary).PageNumbers.Add(PageNumberAlignment=constants.wdAlignPageNumberCenter)  
...
def xtables():
    ...
    for k in range (0, 22): 
        ...
        df3=df3.drop(['GR','NN'], axis=1)
        n_rows, n_cols = df3.shape[0], df3.shape[1]  
        #picPath = "D:\\IPYNB\\KSFE2023\\OUTPUT\\GRAPH" + str(k+1) + ".png"
        picPath = ".\\OUTPUT\\GRAPH" + str(k+1) + ".png"
        GraphKopf="Grafik â„–"+str(k+1)

        doc.add_heading("Bericht Block â„–" +str(k+1)) 
        doc.add_heading("Text Block â„–"+str(k+1),  2) 
        p = doc.add_paragraph("Hinweis: Alle Text BlÃ¶cke wurde vom Python-Dokumentation kopiert!")
        for run in p.runs:
            run.bold = True
            run.underline = True
            run.italic = True
        ...
        doc.add_heading("Tabelle â„–"+str(k+1), 2) 
        doc.add_heading("Altersverteilung fÃ¼r ausgewÃ¤hlte LÃ¤nder nach WHO: " + str(country.values[k, 1]), 3) 
        ...
        cell.paragraphs[0].add_run("Â© Dr. Alexander Wagner. All Rights Reserved")
        cell.vertical_alignment = WD_CELL_VERTICAL_ALIGNMENT.CENTER 
        ...
        doc.add_section()
    doc.add_heading("Bericht Block â„–23", 1) 
      
    for tb in range(8):  
        k=345
        s=22
        print(tb)
        doc.add_heading("Text Block â„–" + str(s+1+tb), 3) 
        p = doc.add_paragraph("Hinweis: Alle Text BlÃ¶cke wurde vom Python-Dokumentation kopiert!")
        for run in p.runs:
            run.bold = True
            run.underline = True
        ...
        for j in range(0, 4):
            ...
            #tab_copy(tb)
        if tb < 7:
            p2 = doc.add_paragraph("Die Tabelle â„–"  + str(s+tb+1) + " wurde erfolgreich kopiert!")
    ...
    #doc.save("D:\\IPYNB\\KSFE2023\\OUTPUT\\AVaS2023Test.docx")
...
def post_page():
    ...
    paragraph = cell.paragraphs[0]
    run = paragraph.add_run()
    #run.add_picture("D:\\IPYNB\\KSFE2023\\INDATEN\\KI3.jpg")
    run.add_picture(".\\INDATEN\\KI3.jpg")
    paragraph.alignment = WD_ALIGN_PARAGRAPH.RIGHT

    cell = hetable.rows[0].cells[0]
    paragraph = cell.paragraphs[0]    
    paragraph.text = "Projektinitiative: Ein Web- und Cloudbasiertes Multiple-Kernel Eco-System fÃ¼r die automatisierte Erstellung von analytischen Berichten.\nEntwickelt von Dr. Alexander Wagner im November 2022"
    ...
    footer_para.add_run('Datum der Berichtserstellung: ' + dato + ', Programm-Version: â„–' + str(version) + '\t\t') 
    footer_para.alignment = WD_ALIGN_PARAGRAPH.LEFT
    footer_para.add_run('\nDocument owner: ' + docOwner + '\t\t')
    footer_para.alignment = WD_ALIGN_PARAGRAPH.LEFT
    footer_para.add_run("\nÂ© Paradox GmbH. All Rights Reserved \t\t\t\t")
...
def open_doc():
    docx_file = ".\\OUTPUT\\xAVaS2023Finish.docx" 
...
docx_file=os.path.join(path, "", "OUTPUT", "xAVaS2023Finish.docx")
print(docx_file)
doc.save(docx_file)
